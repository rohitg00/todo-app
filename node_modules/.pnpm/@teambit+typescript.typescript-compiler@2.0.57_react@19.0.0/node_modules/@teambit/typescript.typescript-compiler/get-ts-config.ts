...
export function computeTsConfig({
  tsconfig,
  compilerOptions,
}: ComputeTsConfigOptions) {
  if (compilerOptions) {
    return {
      compilerOptions,
    };
  }
  const tsconfigContents = tsconfig
    ? getTsconfig(dirname(tsconfig), basename(tsconfig))
    : undefined;
  if (tsconfigContents?.config) return tsconfigContents?.config;
  throw new TsConfigNotFound(tsconfig);
}
