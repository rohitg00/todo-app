...
export class TypescriptCompiler implements Compiler {
  ...
  async preGenerateTypesOnWorkspace(
    params: TypeGeneratorCompParams[],
    envId: string
  ): Promise<void> {
    ...
    await Promise.all(
      params.map(async (param) => {
        await fs.writeFile(
          path.join(param.packageDir, 'tsconfig.json'),
          this.stringifyTsconfig(customTsconfig)
        );
      })
    );
    await this.writeTypes(
      params.map((param) => param.packageDir),
      envId
    );
  }

  private createHost(currentComponentResult: Partial<ComponentResult>) {
    const formatHost = {
    ...
    const reportDiagnostic = (diagnostic: typescript.Diagnostic) => {
      const errorStr = process.stdout.isTTY
        ? this.tsModule.formatDiagnosticsWithColorAndContext(
            [diagnostic],
            formatHost
          )
      ...
      if (!currentComponentResult.component || !currentComponentResult.errors) {
        throw new Error(
          `currentComponentResult is not defined yet for ${diagnostic.file}`
        );
      }
  ...
  async generateTypesOnWorkspace(
    rootDir: string,
    params: TypeGeneratorCompParams[]
  ) {
    ...
    await this.writeProjectReferencesTsConfig(rootDir, packageDirs);
    const solutionBuilder = this.tsModule.createSolutionBuilder(
      host,
      [rootDir],
      { verbose: true }
    );
    ...
    while ((nextProject = solutionBuilder.getNextInvalidatedProject())) {
      ...
      const foundParam = params.find(
        (param) => param.packageDir === packagePath
      );
      if (!foundParam)
        throw new Error(
          `generateTypesOnWorkspace: unable to find params for ${packagePath}`
        );
      ...
      nextProject.done(
        undefined,
        undefined,
        this.options.typescriptTransformers
      );
  ...
  transpileFile(
    ...
    const compilerOptionsFromTsconfig =
      this.tsModule.convertCompilerOptionsFromJson(
        this.rawTsConfig.compilerOptions,
        '.'
      );
  ...
  private concatDistDirIfNeeded(
    outputFiles: TranspileFileOutput
  ): TranspileFileOutput {
    ...
    const withDistDirs = outputFiles.map((outputFile) => {
      return {
        ...
        outputPath: normalize(path.join(this.distDir, outputFile.outputPath)),
      };
    });
    return withDistDirs;
  }
  ...
  isFileSupported(filePath: string): boolean {
    ...
    const isJsxAndCompile =
      !!this.options.compileJsx && filePath.endsWith('.jsx');
    const isSupported =
      (['.ts', '.tsx', '.mts', '.cts', '.mtsx', '.ctsx'].some((ext) =>
  ...
  private async runTscBuild(network: Network): Promise<ComponentResult[]> {
    ...
    while ((nextProject = solutionBuilder.getNextInvalidatedProject())) {
      ...
      const capsulePath = resolve(nextProject.project, '..');
      // regex to make sure it will work correctly for both linux and windows
      ...
      const currentComponentId =
        network.graphCapsules.getIdByPathInCapsule(capsulePath);
      ...
      if (!capsule)
        throw new Error(
          `unable to find capsule for ${currentComponentId.toString()}`
        );
  ...
  private removeDuplicateTypes(envId: string) {
    ...
    const filenames = this.options.types.map((typePath) =>
      path.basename(typePath)
    );
    const duplicatedFilenames = filenames.filter(
    ...
    uniq(duplicatedFilenames).forEach((filename) => {
      ...
      const fullPaths = this.options.types.filter(
        (typePath) => path.basename(typePath) === filename
      );
      ...
      this.options.types = this.options.types.filter(
        (typePath) => !pathsToRemove.includes(typePath)
      );
    });
    this.logger
...
It's recommended to fix the env (${envId}) configuration to have only one file per type.`);
  ...
  private async writeNpmIgnore(dirs: string[]) {
    ...
    await Promise.all(
      dirs.map(async (dir) => {
        const tsconfigFiles = await glob('tsconfig.*.json', { cwd: dir });
        const npmIgnorePath = path.join(dir, NPM_IGNORE_FILE);
        const foundRootTsConfigEntries = tsconfigFiles.map(
          (tsconfig) => `/${tsconfig}`
        );
        const npmIgnoreEntries = uniq([
          `${this.distDir}/tsconfig.tsbuildinfo`,
          `${this.distDir}/tsconfig.json`,
        ...
        if (await fs.pathExists(npmIgnorePath)) {
          existingEntries = (await fs.readFile(npmIgnorePath, 'utf8')).split(
            '\n'
          );
        }
        const filtered = difference(npmIgnoreEntries, existingEntries);
  ...
  private async writeProjectReferencesTsConfig(
    rootDir: string,
    projects: string[]
  ) {
    const files: string[] = [];
    const references = projects.map((project) => {
      const projectPath = this.options.capsuleTsConfigName
        ? path.join(project, this.options.capsuleTsConfigName)
        : project;
  ...
  private async writeTsConfig(capsules: CapsuleList, network: Network) {
    ...
    const hasGetDependenciesAPI = Boolean(
      someCapsule.component.getDependencies
    );
    ...
    const writeTsConfigWithoutProjectReferences = async () => {
      ...
      await Promise.all(
        dirs.map((dir) =>
          fs.writeFile(path.join(dir, 'tsconfig.json'), tsconfigStr)
        )
      );
    };
...
Please run "bit insights" to see the circular dependencies`);
    ...
    await Promise.all(
      capsules.map(async (capsule) => {
        ...
        const depsDirs = compact(deps.map((dep) => idsPathsMap[dep.id]));
        const customTsconfig = { ...this.rawTsConfig };
        const paths = await Promise.all(
          depsDirs.map(async (depDir) => {
            const tsconfigPath = path.join(depDir, 'tsconfig.json');
            ...
            if (tsconfigFiles.includes(tsconfigName)) {
              return path.join(depDir, tsconfigName);
            }
            return tsconfigFiles.map((tsconfigFile) =>
              path.join(depDir, tsconfigFile)
            );
          })
        );
        const flattenedPaths = compact(paths).flat();
        customTsconfig.references = flattenedPaths.map((p) => ({ path: p }));

        customTsconfig.compilerOptions = customTsconfig.compilerOptions || {};
        customTsconfig.compilerOptions.composite = true;
        await fs.writeFile(
          path.join(capsule.path, tsconfigName),
          this.stringifyTsconfig(customTsconfig)
        );
      })
    );
