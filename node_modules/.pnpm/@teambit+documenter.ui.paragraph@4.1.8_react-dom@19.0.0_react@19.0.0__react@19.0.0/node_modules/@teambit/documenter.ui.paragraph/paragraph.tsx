 ...
 */
export function Paragraph({ children, className, size = 'md', element = 'p', ...rest }: ParagraphProps) {
  const paragraphSize = size ? PossibleSizes[size] : PossibleSizes.md;
  return (
    <BaseParagraph element={element} size={paragraphSize} {...rest} className={classNames(styles.paragraph, className)}>
      {children}
    </BaseParagraph>
  );
}